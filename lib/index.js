...
function setupApp () {
  ...
  app.all('*', (req, res) => {
    ...
    console.log(`clientId is ${clientId}, instanceId is ${instanceId}`)
    let mapping = null
    if (config.mapping) {
      config.mapping.forEach((map) => {
        console.log(`comparing clientID _${map.clientID}_ and instanceID _${map.instanceID}_`)  
    ...
    if (mapping) {
      ...
      console.log(`sending basic auth with username ${mapping.username}`)
    } else {
      console.log(`mapping is not defined. proceeding without authentication`)
    }
    ...
    request(options, (err, upstreamRes, upstreamBody) => {
      ...
      res.send(utils.buildReturnObject(mediatorConfig.urn, 'Successful', 200, upstreamRes.headers, upstreamRes.body.toString(), orchestrations))
    })
  })
  return app
}
